...
def get_length():
    ...
    while True:
        length = (
            input(
                "Enter the length of word you want to see from 4-7 inclusive. If you want to see every answer, type 'all'. To try another word press Enter. To exit type \"exit\": "
            )
            .strip()
            .lower()
        )
...
def validate_length(length):
    try:
        if not length or length == "all" or length == "exit" or 4 <= int(length) <= 7:
            return length
        else:
            raise ValueError()
    except ValueError:
        raise ValueError("Length has to be 4-7 inclusive, 'all', or 'exit'!")
...
def print_answers(answers: list, length: str) -> None:
    ...
    # all the answers
    if length == "exit":
        return "" # Exits and creates newline
    elif length == "all":
...
def find_answers(letters, required):
    ...
    with open(
        os.path.join(__location__, "words_dictionary.json"), encoding="utf-8"
    ) as json_file:
